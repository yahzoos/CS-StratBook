...
func LoadSettings() Settings {
	...
	if err != nil {
		...
		checkTagsFile(s.TagsPath)
		return s
	}

	// Parse JSON
	if err := json.Unmarshal(data, &s); err != nil {
		fmt.Println("Error parsing settings.json, using defaults:", err)
		s = Settings{
			TagsPath:       "tags.json",
			AnnotationPath: filepath.Join("C:\\", "Program Files (x86)", "Steam", "steamapps", "common", "Counter-Strike Global Offensive", "game", "csgo", "annotations"),
		}
...
func checkTagsFile(filename string) {
	if _, err := os.Stat(filename); err == nil {
		// File exists
		log.Println("Tags file exists:", filename)
	} else if os.IsNotExist(err) {
		// File does not exist → create it
		log.Println("Tags file does not exist. Creating:", filename)
		...
		log.Println("Error checking tags file:", err)
	}
}
